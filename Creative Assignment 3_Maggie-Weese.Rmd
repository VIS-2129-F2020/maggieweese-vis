...
All the data retrieved was from https://data.baltimorecity.gov/browse?provenance=official&q=neighborhoods&sortBy=relevance&page=2. From the site, I loaded two polygon data sets, neighborhood and vegetated area, and two point data sets, Charm City circulator bus stops, schools and library locations. 
```{r, include=FALSE}
...
ggplot(neighborhoods) +
  ...
  geom_sf(data = busstop, size = 1,
          aes(color = by_libs)) +
  scale_color_manual(values = c("turquoise2", "blue"),
...
ggplot(neighborhoods) + 
  ...
  scale_fill_viridis_c(option = "viridis", direction = 1, name = "Baltimore neighborhoods by \nnumber of public schools",
...
ggplot(neighborhoods) + #plot neighborhood map
 geom_sf(fill = "grey97", color = "grey90") + #choosing colors of lines (fill) and background (color)
  geom_sf(data = schools, size = 1.5, #make the data points that appears be based on spatial data of busstop with size 1
  ...
  annotate(geom = "text", x = left_side,  #put this text at x I specified above
           ...
           label = paste("Of the ", #text to include there
                        ...
                        " (", prettyNum(100*pct_schools_libs, digits = 0), #create percentage based on that number above
                         ...
                         sep = ""),
           hjust = 0, vjust = 7.25, size = 4) + #size of font and location adjustment
...
ggplot(neighborhoods) +
  geom_sf(color = NA, 
    ...
    scale_fill_viridis_c(option = "inferno", direction = -1, name = 
...
ggplot(veggie) +
  ...
  geom_sf(data = neighborhoods,
          aes(fill = has_veggie)) +
  scale_fill_manual(values = c("cornsilk1", "mediumseagreen"),
          name = "Baltimore Neighborhoods\nby presence of Vegetated Area", 
...
ggplot(neighborhoods) + 
  geom_sf(color=NA,
          aes(fill=num_bus)) +
  scale_fill_viridis_c(option = "viridis", direction = 1, name = "Baltimore neighborhoods by \nnumber of Charm City Circulator Stops",
    ...
    theme(legend.background = element_rect(fill = alpha("white", 0.5),
     ...
     annotate(geom = "text", x = left_side, 
           ...
           label = paste("  Balltimore City's ",
                         ...
                         " free bus\n service stops are concentrated in\n    24 neighborhoods.",
                         sep = ""),
           hjust = -0.1, vjust = 2.5, size = 4, color = "white")
```
