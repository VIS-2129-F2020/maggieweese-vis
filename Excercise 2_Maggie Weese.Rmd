...
ggplot(covid_countries, aes(fill = newDeath, label = "Country_Region")) +
  geom_sf(color = NA) +
  ...
  geom_sf_text(aes(label = name), size=3, color = "darkslategray4", inherit.aes = FALSE) +
  annotation_scale() + 
...
```{r}
covid_countries[is.na(covid_countries)] <- 0

covid_dorling <- covid_countries %>%
